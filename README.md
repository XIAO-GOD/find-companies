...
- 检索产品、技术路线、团队、融资、客户、专利、招聘、竞争和风险信息，并在结束前执行最新信息复查
- 区分“已证实”“公司自述”“分析判断”“未确认”四类证据状态
- 强制记录每项结论的有效日期、时效范围和重要性，防止历史信息被写成当前状态
- 搜索摘要、不可访问页面、公司单方口径和聚合数据库不能被标成独立证实
- 关键结论必须由两个不同发布者的当前可访问来源交叉验证，否则自动拒绝渲染或降级
...
默认输出是一份带“证据与新鲜度审计”的中文 HTML 拜访简报。研究超过7天、使用未来日期、引用搜索结果页或把公司自述写成已证实时，渲染器会直接报错。它服务于初步了解与是否继续跟进的判断，不替代完整的财务、法律、专利或客户尽调。
...
scripts/test_render_brief.py     幻觉与过期信息回归测试
```
...
公开信息不足时，Skill 会明确展示未知项，不会补写未经证实的收入、估值、客户、订单、良率、性能或市场份额。搜索不到信息只表示“本次未找到”，不等于该事项不存在。
